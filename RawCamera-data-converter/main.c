...
static int bytesPerPixel(uint8_t alg){
...
uint8_t readImg(uint32_t numf,uint16_t offset,uint8_t * dat,uint8_t alg,char * fileName){
	FILE * myfile;
	if(fileName==0){
		sprintf_s(buf, 128, "%d.RAW",numf);
		fopen_s(&myfile, buf, "rb");
	}else
...
int main(int argc,char ** argv){
	...
	uint8_t * Dat = 0;//in case some of the file was not saved we use calloc instead of malloc to guarantee that the unsaved pixels are set to 0
	if(useNum!=2)
	...
	switch(useNum){
		...
		sprintf_s(buf, 128, "frame %d.png", useImg);
		...
		case 2:
			...
			sprintf_s(buf, 128, "frame %d-%d.png", useImg, useImg + numImg - 1);
			if(savePNG(buf,img_wo,img_ho,outImg)){
				puts("Error while saving PNG");
				return 1;
			}}
		...
		case 3:
			...
			sprintf_s(buf, 128, "%s.png", fileName);
			if (savePNG(buf,img_wo,img_ho,outImg)){
				puts("Error while saving PNG");
				return 1;
			}
		...
		default:
			...
			for(;;++imgC){
				...
				if(processImg(Dat,outImg,imgC,debayer,offset,sqrtUse,sineUse,0,0))
					break;
				sprintf_s(buf, 128, "frame %d.png", imgC);
				if(savePNG(buf,img_wo,img_ho,outImg)){
					puts("Error while saving PNG");
					return 1;
				}
